 ...
 * Popup bộ lọc cột - chọn cột nào hiển thị, kéo thả sắp xếp
 */
...
const FilterPopup = {
    ...
    createPopup() {
        ...
        this.popup.innerHTML = `
            <div class="popup-header">
                <h3 class="popup-title">Bộ lọc</h3>
                <button class="btn-close" onclick="FilterPopup.close()">
                    <i class="icon-default icon-close"></i>
                </button>
            </div>
            <div class="popup-body">
                <div class="form-group" style="margin-bottom: 16px;">
                    <input type="text" class="form-input" id="filterColumnSearch" placeholder="Tìm kiếm...">
                </div>
                <div class="column-actions" style="margin-bottom: 16px;">
                    <button class="btn btn-secondary" onclick="FilterPopup.selectAll()" style="margin-right: 8px;">Chọn tất cả</button>
                    <button class="btn btn-secondary" onclick="FilterPopup.deselectAll()">Bỏ chọn tất cả</button>
                </div>
                <div class="column-list" id="filterColumnList">
                    ${this.renderColumnList()}
                </div>
            </div>
            <div class="popup-footer">
                <button class="btn btn-secondary" onclick="FilterPopup.close()">Hủy</button>
                <button class="btn btn-primary" onclick="FilterPopup.apply()">Áp dụng</button>
            </div>
        `;
        ...
        return this.columns.map((col, index) => `
            <div class="column-item" draggable="true" data-index="${index}">
                <input type="checkbox" class="checkbox" ${col.visible ? 'checked' : ''}
                    onchange="FilterPopup.toggleColumn(${index}, this.checked)">
                <span>${col.label}</span>
            </div>
        `).join('');
    },
    ...
    refreshColumnList() {
        const columnList = document.getElementById('filterColumnList');
        if (columnList) {
    ...
    bindEvents() {
        ...
        document.addEventListener('input', (e) => {
            if (e.target.id === 'filterColumnSearch') {
    ...
    apply() {
        ...
        localStorage.setItem('filterSettings', JSON.stringify(this.columns));

        // Update table headers and cells based on filter
    ...
    updateTableDisplay() {
        ...
        // Column index mapping (skip checkbox column at index 0)
        const columnMap = {
            'phone': 1,
            'source': 2,
            'name': 3,
        ...
        // Update header order based on filter settings
        const thead = table.querySelector('thead tr');
        const headerCells = Array.from(thead.querySelectorAll('th'));
        const checkboxHeader = headerCells[0]; // Keep checkbox column

        // Sort columns by order
        const sortedColumns = [...this.columns].sort((a, b) => a.order - b.order);

        // Clear and rebuild header (keep checkbox)
        thead.innerHTML = '';
        thead.appendChild(checkboxHeader);

        sortedColumns.forEach(col => {
            const colIndex = columnMap[col.id];
            if (colIndex !== undefined && headerCells[colIndex]) {
                const header = headerCells[colIndex].cloneNode(true);
        ...
        rows.forEach(row => {
            ...
            const checkboxCell = cells[0]; // Keep checkbox column

            // Clear and rebuild row (keep checkbox)
            row.innerHTML = '';
